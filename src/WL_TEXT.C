...
^P[enter]				start _new page, must be the first chars in a layout
...
void HandleCommand (void)
{
	...
	case 'G':		// ^Gyyy,xxx,ppp draws graphic
		...
		if (picmid > SCREENMID)
			margin = picx-PICMARGIN;			// _new right margin
		else
			margin = picx+picwidth+PICMARGIN;	// _new left margin
...
void HelpScreens (void)
{
	...
	MM_SetLock (&grsegs[artnum], _true);
#else
	CA_LoadFile (helpfilename,&layout);
	text = (char _seg *)layout;
	MM_SetLock (&layout, _true);
#endif
