...
//			difficulty levels if a _new game should be started
...
US_Startup(void)
{
	...
	US_InitRndT(_true);		// Initialize the random number generator
...
//		_true), nothing is copied into buf, and _false is returned. If the
//		user hits return, the current string is copied into buf, and _true is
//		returned
//
///////////////////////////////////////////////////////////////////////////
boolean
				...
				int maxchars,int maxwidth)
{
	...
	*olds = '\0';
	cursor = strlen(s);
	cursormoved = redraw = _true;

	cursorvis = done = _false;
	lasttime = TimeCount;
	LastASCII = key_None;
	LastScan = sc_None;
