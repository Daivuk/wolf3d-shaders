...
= SpawnNewObj
=
= Spaws a _new actor at the given TILE coordinates, with the given state, and
= the given size in GLOBAL units.
=
= _new			= a pointer to an initialized _new actor
=
===================
*/
...
void SpawnNewObj (unsigned tilex, unsigned tiley, statetype *state)
{
	...
	_new->state = state;
	if (state->tictime)
		_new->ticcount = US_RndT () % state->tictime;
	else
		_new->ticcount = 0;

	_new->tilex = tilex;
	_new->tiley = tiley;
	_new->x = ((long)tilex<<TILESHIFT)+TILEGLOBAL/2;
	_new->y = ((long)tiley<<TILESHIFT)+TILEGLOBAL/2;
	_new->dir = nodir;

	actorat[tilex][tiley] = _new;
	_new->areanumber =
		*(mapsegs[0] + farmapylookup[_new->tiley]+_new->tilex) - AREATILE;
}
...
= NewState
=
= Changes ob to a _new state, setting ticcount to the max for that state
=
===================
*/
...
= Otherwise
=
= ob->dir			= _new direction to follow
...
=
= ob->x			= adjusted for _new position
= ob->y
=
=================
*/
...
= CheckLine
=
= Returns _true if a straight line between the player and ob is unobstructed
=
=====================
*/
...
=
= returns _true if the player has been spoted
=
================
*/
...
= it's combat frame and _true is returned.
=
